...
var ProspectWorkspace = (function () {
    ...
    function overview(rec, candidate) {
        ...
        var estimate = typeof ProspectCapital !== 'undefined' ? ProspectCapital.estimate(candidate || { id: rec.id, energyType: rec.energy_type, sourceDetail: {} }, rec) : null;
        if (estimate) { cap = estimate.capacity; b = estimate.budget; }
        return (estimate ? ProspectCapital.summary(estimate, true) : '') + '<section class="pw-next"><span class="pw-eyebrow">' + esc(closed(rec.stage) ? 'Recorded outcome' : 'Next step') + '</span><h3>' + esc(n.text) + '</h3><p>' + esc(p ? p.why : 'Keep the agreement, decision and supporting records with this site.') + '</p><div class="pw-actions"><button type="button" class="pw-primary" data-open-panel="' + esc(n.panel) + '"' + (n.focus ? ' data-focus="' + esc(n.focus) + '"' : '') + '>Open ' + esc(detailTabs.find(function (t) { return t[0] === n.panel; })[1].toLowerCase()) + ' →</button>' + (n.due ? '<span>' + esc(dueLabel(n.due)) + '</span>' : '') + '</div></section>' +
            '<div class="pw-keyfacts"><div><span>Power allocated to Proton</span><strong>' + kw(estimate ? estimate.capacity.contractedKw : cap.contractedKw) + '</strong><small>Documented net power for this deal</small></div><div><span>Reviewed budget</span><strong>' + usd(b.base) + '</strong><small>' + (b.complete ? 'All lines addressed; recorded costs and allowances' : 'Budget incomplete: ' + (b.missing || []).length + ' components still need evidence') + '</small></div></div>' +
    ...
    function enhanceDetail(rec, host, candidate) {
        ...
        var ctx = { saved: rec, findSaved: function (id) { return SiteData.get(id); }, onSave: function () {
            ...
            var editors = ProspectDiligenceUi.openState(panels.capital);
            panels.capital.innerHTML = '<div id="dtab_capacity">' + ProspectDiligenceUi.renderBuckets(c, ctx).capacity + '</div>';
            ProspectDiligenceUi.bind(panels.capital, c, ctx); ProspectDiligenceUi.restoreOpen(panels.capital, editors);
            panels.overview.innerHTML = overview(rec, candidate);
