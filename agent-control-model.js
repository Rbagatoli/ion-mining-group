...
(function (root, factory) {
    ...
    var OFFERS = { quote_review: 'Quote & Cost Review', research: 'Supplier Prospect Research', sourcing: 'Sourcing Desk', brief: 'Site Decision Brief', retainer: 'Ongoing service' };
    ...
    function reduce(before, a) {
        ...
        case 'task.result':
            ...
            t.result=str(p.result,18000,'result',true);t.sources=sources(p.sources||[]);t.status='review';t.blocker='';t.reviewNote='';t.updatedAt=a.at;message='Result submitted for review: '+t.title;break;
        case 'task.block':
            t=get(s,'tasks',p.id);if(['done','cancelled'].includes(t.status))fail('This task is closed.');
            t.status='blocked';t.blocker=str(p.reason,2000,'blocker',true);t.updatedAt=a.at;message='Task blocked: '+t.title;break;
        case 'task.accept':
        case 'task.revise':
    ...
    function metrics(s, month) {
        ...
        out.open=s.tasks.filter(function(t){return !['done','cancelled'].includes(t.status);}).length;
        out.review=s.tasks.filter(function(t){return t.status==='review';}).length;
    ...
    return {ROLES:ROLES,OFFERS:OFFERS,LEAD_STAGES:LEAD_STAGES,CHANNELS:CHANNELS,STATUS:STATUS,STAGES:STAGES,KINDS:KINDS,COMMON:COMMON,initial:initial,valid:valid,reduce:reduce,metrics:metrics,leadMetrics:leadMetrics,packet:packet,kickoff:kickoff,url:url};
}));
