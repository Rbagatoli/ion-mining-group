/* ===== PROTON MINING — the gas field =====
   ...
   Stranded energy combusting and rising, behind the operator app.

   THE THIRD COPY, and the duplication is deliberate for the same reason the
   second one is. site/hero-anim.js draws this on all thirteen marketing
   backdrops, sized to an element; portal/gas-field.js draws it behind the
   producer sign-in, sized to a viewport; this draws it behind the app. The
   three cannot be one file because they are three deploy roots — the build
   publishes site/ at /, the portal at /portal/ and the app at /app/, so a
   shared copy one level up resolves to nothing in two of them.

   What keeps them honest is not discipline, it is assertions:
   tests/portal-frontend.test.js pins the ink constants and the motion
   constants across all three files rather than inside any one of them, so they
   cannot drift apart quietly.

   WHAT THIS REPLACES. A field of 45 straight lines, 30 nodes and 12 travelling
   pulses — a "tech lines" backdrop that was the last thing in the app not
   drawn from the same substance as everything else. The site has one animation
   and it is this one: energy with no customer, travelling upward, cooling
   orange to platinum.
