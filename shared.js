...
function initNav(activePage) {
    ...
    // The theme toggle lived here. Removed with the light theme -- see isLightMode above.
    // The stored preference is cleared once, behind a sentinel, so a returning user is not left
    // with a dead key. The Firestore document is deliberately NOT deleted: a user still on the
    // old build who syncs afterwards would read null, fall back to 'dark', and have their theme
...
// ===== BACKGROUND =====
// The tech-lines field lived here: 45 straight lines, 30 nodes and 12 travelling
// pulses, about 235 lines of canvas. It was the last thing in the app not made of
// the same substance as everything else. gas-field.js replaces it with the
// animation the marketing site and the portal both already run -- stranded energy
// rising and cooling from orange to platinum -- so one animation covers all three
// surfaces. Its resize and visibilitychange handlers went with it; the gas field
// brings its own, plus a composed still for prefers-reduced-motion rather than the
// blank box the old reduced-motion rule left behind.
