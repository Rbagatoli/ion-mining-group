...
(function () {
  ...
  function changed() { return Object.keys(defaults).filter(k => !['company', 'role', 'notes', 'known'].includes(k)).some(k => state.brief[k] !== defaults[k]); }
  ...
  function overview() {
    main.innerHTML = heading('YOUR NEXT CHAPTER', 'Good energy. A better starting point.', 'A place to turn possible sites into an informed next move.', button('Edit my brief', 'brief')) +
      ...
      <div class="overview-bottom"><section class="panel compact-panel"><p class="eyebrow">BUILT AROUND YOUR BRIEF</p><h3>${esc(state.brief.region)}</h3><p>${esc(state.brief.infrastructure)} · ${esc(state.brief.timing)}</p></section><section class="panel compact-panel"><p class="eyebrow">NEXT ACTION</p><h3>Which sites deserve a closer look?</h3><p>Save your feedback and prepare one clear request for Proton. <a class="text-link" href="#updates">Review updates →</a></p></section></div>`;
  }
  ...
  function brief() {
    main.innerHTML = heading('01 / YOUR REQUIREMENTS', 'What makes a site work for you?', 'Start with the essentials. We’ll use the brief to agree the scope of your search.') +
      ...
      <section class="form-section"><h2>Power &amp; capital</h2><div class="form-grid">${field('minMw', 'Minimum load · MW', 'number', 'min="0.001" step="any" max="10000" required')}${field('maxMw', 'Maximum load · MW', 'number', 'min="0.001" step="any" max="10000" required')}${field('budget', 'Site capital budget · USD', 'number', 'min="1" max="10000000000" step="any" placeholder="Optional"', 'Excluding miners. Leave blank if undecided.')}${field('rate', 'Energy-only target · US ¢/kWh', 'number', 'min="0.001" max="1000" step="any" placeholder="Optional"', 'A target, not a rate available from Proton.')}${select('infrastructure', 'Infrastructure preference', ['Existing generation preferred', 'Installation-ready only', 'Open to new construction'])}${select('timing', 'Target start', ['Flexible', 'Within 3 months', '3–6 months', '6–12 months', 'More than 12 months'])}</div></section>
  ...
  function briefText() {
    const b = state.brief;
    return ['Energy site search brief', `Company/project: ${b.company || 'Not supplied'}`, `Client type: ${b.role}`, `Location: ${b.region}`, `Load: ${b.minMw}–${b.maxMw} MW`, `Site capital budget (USD, excluding miners): ${money(b.budget)}`, `Energy-only target (US cents/kWh): ${b.rate || 'Not set'}`, `Infrastructure preference: ${b.infrastructure}`, `Timing: ${b.timing}`, `Already-known sites: ${b.known || 'None supplied'}`, `Other requirements: ${b.notes || 'None supplied'}`].join('\n');
  }
  ...
  document.addEventListener('click', e => {
    ...
    const tab = e.target.closest('[data-tab]'); if (tab) { activeTab = tab.dataset.tab; renderDetail(); document.querySelector(`[data-tab="${activeTab}"]`).focus({ preventScroll: true }); return; }
