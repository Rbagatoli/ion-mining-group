...
function packageBytes(rel,bytes){
  ...
  return /\.(?:m?js|css|html|json|svg|webmanifest|md|txt)$/i.test(rel)||/(?:^|\/)LICENSE(?:\.[^/]*)?$/i.test(rel)
...
function assets(){
  ...
  return [{from:'site/intake-config.js',to:'intake-config.js'},...shell.map(p=>({from:'crm/'+p,to:p})),...exchange.map(p=>({from:'crm/agent-exchange/'+p,to:'agent-exchange/'+p})),...runtime.concat(globe).map(p=>({from:p,to:'runtime/'+p})),{from:'site/vendor/three-0.185.1/OrbitControls.js',to:'runtime/globe-assets/vendor/three-0.185.1/OrbitControls.js'},...data.map(p=>({from:p,to:p}))];
}
function validateModuleImports(entries){
  const declared=new Set(entries.map(a=>path.resolve(ROOT,a.to)));
  for(const asset of entries.filter(a=>/\.m?js$/.test(a.to))){
    const file=path.resolve(ROOT,asset.from),body=fs.readFileSync(file,'utf8');
    // The browser bridge uses literal relative imports. Keep that dependency
    // closure inside the explicit public asset list before writing any output.
    for(const match of body.matchAll(/\b(?:import\s*(?:\(\s*)?|(?:import|export)\s+[^;'"`]*?\s+from\s*)(["'])([^"'\r\n]+)\1/g)){
      const specifier=match[2],rel=specifier.split(/[?#]/,1)[0];
      if(!asset.to.endsWith('.mjs')&&!rel.endsWith('.mjs'))continue;
      if(!/^\.\.?\//.test(rel)||!declared.has(path.resolve(ROOT,path.dirname(asset.to),rel)))throw new Error('Undeclared CRM module import in '+asset.from+': '+specifier);
    }
  }
}
function build(destination){
  ...
  const scripts=entries.filter(a=>/\.m?js$/.test(a.to));
  ...
  fs.writeFileSync(html,fs.readFileSync(html,'utf8').replace(/((?:src|href)="\.\/([^"?]+\.(?:css|m?js)))(?:\?[^"\s]*)?"/g,(_,prefix,rel)=>{
