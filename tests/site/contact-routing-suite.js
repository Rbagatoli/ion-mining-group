...
function harness(search='',options={}){
 const listeners={},topicListeners={},navigation=[],attrs={'data-mailto':options.to||'sales@protonminingco.com','data-subject':options.subject||'Enquiry via protonminingco.com'},button={textContent:'Send message',disabled:!!options.disabled};
...
check('site-sourcing deep link chooses the sales inbox and its specific subject',()=>{
 const h=harness('?topic=site-sourcing');assert.equal(h.topic.value,'site-sourcing');assert.equal(h.attrs['data-mailto'],'sales@protonminingco.com');assert.equal(h.attrs['data-subject'],'Energy site sourcing enquiry via protonminingco.com');assert.deepEqual(h.navigation,[]);
});
check('site sourcing composes an encoded draft only when the valid form is submitted',()=>{
 const h=harness('?topic=site-sourcing');h.submit();assert.equal(h.navigation.length,1);const url=new URL(h.navigation[0]);assert.equal(url.protocol,'mailto:');assert.equal(url.pathname,'sales@protonminingco.com');assert.equal(url.searchParams.get('subject'),'Energy site sourcing enquiry via protonminingco.com');assert.match(url.searchParams.get('body'),/Name: Example & Co\./);assert.match(url.searchParams.get('body'),/Message: A 5 MW site in Alberta\.\nBudget & timing to discuss\./);assert.equal(url.searchParams.size,2);assert.equal(h.button.textContent,'Opening your mail app…');
});
check('all public enquiry topics use the sales inbox while preserving their specific subjects',()=>{
 const h=harness('?topic=site-sourcing');
 for(const [topic,subject] of [['hosting','Hosting enquiry'],['energy','Site / energy enquiry'],['managed-hosting','Managed Energy Hosting enquiry'],['partnership','Partnership enquiry'],['media','Media enquiry'],['other','Enquiry'],['site-sourcing','Energy site sourcing enquiry']]){h.change(topic);assert.equal(h.attrs['data-mailto'],'sales@protonminingco.com');assert.equal(h.attrs['data-subject'],subject+' via protonminingco.com');}
 assert.deepEqual(h.navigation,[]);
});
check('unknown deep links preserve the ordinary hosting default',()=>{const h=harness('?topic=unrecognized');assert.equal(h.topic.value,'hosting');assert.equal(h.attrs['data-mailto'],'sales@protonminingco.com');});
...
check('a standalone energy-search brief preserves its data-mailto route without a topic selector',()=>{
 const h=harness('',{topic:false,to:'sales@protonminingco.com',subject:'Energy site sourcing enquiry via protonminingco.com'});h.submit();const url=new URL(h.navigation[0]);assert.equal(url.pathname,'sales@protonminingco.com');assert.equal(url.searchParams.get('subject'),'Energy site sourcing enquiry via protonminingco.com');
});
check('source preferences include only checked enabled controls and use their visible labels',()=>{
 const h=harness('',{topic:false,to:'sales@protonminingco.com'});
...
check('only opted-in draft buttons are enabled after attaching the submit handler',()=>{
 const draft=harness('',{topic:false,enableDraft:true,disabled:true,to:'sales@protonminingco.com'});assert.equal(draft.button.disabled,false);assert.deepEqual(draft.navigation,[]);
