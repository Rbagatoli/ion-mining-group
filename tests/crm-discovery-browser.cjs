...
(async()=>{
 ...
 await check('invalid ranges and no matches have a visible recovery action',async()=>{
  await page.locator('#discoveryMinMw').fill('30');await page.locator('#discoveryMinMw').dispatchEvent('change');await page.locator('#discoveryError').waitFor({state:'visible'});assert.match(await page.locator('#discoveryError').innerText(),/Minimum capacity/);await input('discoveryMinMw','0.25');await input('discoverySearch','NoSuchSite999ZZZ');assert.equal(await page.locator('#discoveryList .row').count(),0);assert.equal(await page.locator('#discoveryGlobe').getAttribute('data-prospect-markers'),'0');await click('clear-all');await settled();await select('discoverySupplyScope','all');assert.equal(await page.locator('#discoveryKind').inputValue(),'specialty');
 });
