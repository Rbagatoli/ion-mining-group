...
The desktop order strip arranges selected machines, fleet totals, energy estimates and checkout across the width below the miner preview. Its “Cost breakdown” disclosure retains the complete itemisation, assumptions and payment timing. Hardware quote requirements and unknown-power messages stay visible in the summary. The energy summary copies the existing shared itemisation rather than calculating another amount; prepaid values remain labelled estimates with their rate assumptions. Smaller screens show a compact order bar while the catalogue is visible and the full order is outside the usable viewport. That bar mirrors the existing order's hardware amount, including partial-price and quote-required qualifications, and carries the selected hosting location into checkout. Its review link leads to the full order; the bar hides when that order comes into view. Exact model names and quantities remain visible, retaining unavailable saved selections as needing review.
