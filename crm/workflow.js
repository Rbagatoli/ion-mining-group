...
(function(root,factory){
  ...
  function detail(lead,state){
    ...
    return '<section class="wf-detail" aria-label="Lead progress">'+(p.closed?'': '<ol class="wf-lead-path">'+steps.map(([id,label],i)=>'<li class="'+(i===index?'is-current':i<index?'is-earlier':'')+'"'+(i===index?' aria-current="step"':'')+'><span>'+String(i+1).padStart(2,'0')+'</span>'+label+'</li>').join('') )+'<div class="wf-now"><small>Now</small><h3>'+escape(p.now)+'</h3><div class="wf-next-grid"><div><small>Responsible</small><strong>'+escape(p.owner)+'</strong></div><div><small>Next action'+(lead.due?' · '+escape(lead.due):'')+'</small><p>'+escape(p.next)+'</p></div></div></div>'+holdBanner(state)+'<div class="wf-linked"><h4>Linked bot work</h4>'+(p.tasks.length?p.tasks.map(t=>'<a href="'+href('task',t.id)+'"><span>'+escape(t.title)+'</span><small>'+escape(taskMeaning(t,state).label)+' · '+escape(role(t.role))+'</small></a>').join(''):'<p>No individual assignment is linked yet. Use “Assign bot work” to make the connection explicit.</p>')+'</div></section>';
  }
