...
(function(root,factory){
  ...
  function bucket(t){
    const kind=A.taskKind(t);if(kind!=='work')return kind;
    if(!active(t))return 'closed';
    if(t.routing?.reviewOwner==='owner')return 'owner';
    if(t.status==='review')return 'review';
    if(t.status==='blocked')return t.blockerKind==='correction'?'correction':'blocked';
    return t.status;
  }
  ...
  function leadProgress(lead,state){
    ...
    const outreach=tasks.find(t=>A.taskKind(t)==='work'&&t.role==='outreach'&&t.status!=='cancelled');
  ...
  function campaigns(state){
    ...
    return [...groups].map(([id,rows])=>{
      ...
      const current=rows.find(r=>bucket(r.task)==='owner')||rows.find(r=>r.task.status==='working')||rows.find(r=>r.task.status==='blocked')||rows.find(r=>r.task.status==='ready')||rows.find(r=>r.task.status==='review')||rows.find(r=>r.task.status==='draft');
  ...
  function holdBanner(state){const h=emailReadiness(state);return '<div class="wf-hold"><span class="wf-dot" aria-hidden="true"></span><div><strong>'+escape(h.label)+'</strong><p>'+escape(h.next)+(h.record?' Checked '+escape(h.record.checkedOn)+'.':' No verification record saved. Account sign-in does not verify email.')+'</p></div><button class="text-button" data-action="email-readiness">View readiness</button></div>';}
  ...
  function offerLabel(lead){return lead.offer==='sourcing'&&/^PM-ASIC-\d+\s*\|\s*ASIC brokerage\s*\|/i.test(lead.notes||'')?'ASIC brokerage':A.OFFERS[lead.offer]||'Revenue lead';}
  function card(lead,state,{expanded=false,content=''}={}){
    const p=leadProgress(lead,state);
    return '<details class="wf-lead-card" data-lead-id="'+escape(lead.id)+'"'+(expanded?' open':'')+'><summary class="wf-lead-summary"><div class="wf-lead-head"><div><strong>'+escape(lead.company)+'</strong><span>'+escape(offerLabel(lead))+'</span></div><span class="tag">'+escape(p.now)+'</span><span class="wf-expand" aria-hidden="true">⌄</span></div><div class="wf-next-grid"><div><small>Responsible</small><strong>'+escape(p.owner)+'</strong></div><div><small>Next action'+(lead.due?' · '+escape(lead.due):'')+'</small><p>'+escape(p.next)+'</p></div></div><span class="wf-note">'+(p.current?'Linked task: '+escape(p.current.title):'No open bot assignment linked')+' · '+escape(when(p.updated))+'</span><span class="wf-toggle-copy"><span class="wf-collapsed-label">Show lead details</span><span class="wf-expanded-label">Hide lead details</span></span></summary><div class="wf-lead-body">'+content+'</div></details>';
  }
