...
(function(root,factory){
  ...
  function render(input,h){
    ...
    const roleRows=A.ROLES.map(r=>{
      const tasks=v.tasks.filter(t=>t.role===r.id),working=tasks.filter(t=>F.bucket(t)==='working').length,review=tasks.filter(t=>F.bucket(t)==='review').length,blocked=tasks.filter(t=>F.bucket(t)==='blocked').length,owners=tasks.filter(t=>F.bucket(t)==='owner').length,corrections=tasks.filter(t=>F.bucket(t)==='correction').length;
      const status=[working?working+' in progress':'',review?review+' in team review':'',owners?owners+' owner decisions':'',corrections?corrections+' corrections':'',blocked?blocked+' blocked':''].filter(Boolean).join(' · ')||(tasks.length?tasks.length+' open':'No open tasks');
