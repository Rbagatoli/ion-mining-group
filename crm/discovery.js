...
(function(root){
  ...
  const defaults=()=>({query:'',location:'',kind:'landfill_gas',country:'',sort:'priority',cash:'',minMw:'',maxMw:'',generation:false,tracking:'',infrastructure:''});
  function create({data:D,model:M,esc,options,row,tag,empty,onSelect,onRestore}){
    ...
    function html(){return '<section id="discoverWorkspace" class="discover-workspace" data-view="'+mode+'" data-expanded="'+detailOpen+'">'+
      ...
      '<p class="quiet-note discover-footnote">MW filters and prices use the same planned mine size. This is not allocated power. Open Capital for reported equipment, possible reuse savings and unpriced work.</p></section>';}
    function filters(){
