...
(function(root){
  ...
  const energy={all:'All energy',grid_facility:'All power facilities',...Object.fromEntries(E.matching.sourceTypes.map(s=>[s.id,s.label]))};
  const defaults=()=>({query:'',location:'',kind:'all',country:'USA',sort:'priority',cash:'',minMw:'',maxMw:'',generation:false,tracking:'',infrastructure:''});
  function create({data:D,model:M,esc,options,row,tag,empty,onSelect,onRestore,onBrief}){
    ...
    function html(){return '<section id="discoverWorkspace" class="discover-workspace" data-view="'+mode+'" data-expanded="'+detailOpen+'">'+
      '<header class="page-heading discover-heading"><div><p class="eyebrow">DISCOVER</p><h1>Find your next site.</h1><p>Screen nationwide energy sources against the client’s needs.</p></div>'+action('Client search brief','client-brief','class="button"')+'</header><div id="discoveryBrief"></div>'+
    ...
    function filters(){
      ...
      el('discoveryBrief').innerHTML=clientBrief?'<div class="banner"><strong>'+esc(clientBrief.client||'Internal sample')+'</strong> · '+esc(clientBrief.minMw)+' MW minimum client allocation · '+esc(E.costBases[clientBrief.costBasis])+'<p>Fit uses dated owner evidence. Public capacity alone stays unresolved. This search brief is temporary until saved with site evidence or a research assignment.</p>'+action('Edit brief','client-brief','class="text-button"')+' '+action('Clear client brief','clear-brief','class="text-button"')+'</div>':'';
    ...
    const fields={discoverySearch:'query',discoveryLocation:'location',discoveryKind:'kind',discoveryCountry:'country',discoverySort:'sort',discoveryCash:'cash',discoveryMinMw:'minMw',discoveryMaxMw:'maxMw',discoveryTracking:'tracking',discoveryInfrastructure:'infrastructure'};
    ...
    async function search(){
      ...
      try{
        ...
        for(let i=0;i<catalog.length;i++){
          ...
          if(S.matchCandidate({...c,energyTypes:match.energyTypes},f,saved)){const x=D.summary(c,saved,cash);if(clientBrief||S.matchCash(x,f)&&S.matchCapacity(x,f)&&S.matchInfrastructure(x,f))rows.push({...x,lat:c.lat,lng:c.lng,potentialKw:c.powerPotentialKw,generation:Number.isFinite(c.existingGenerationKw)&&c.existingGenerationKw>0,place:S.location(c),saved:!!saved,identity,fit:clientBrief?E.assess(c,saved,clientBrief):null,familyLabel:(match.energyTypes||[]).map(id=>energy[id]||id).join(' / ')});}
        ...
        el('discoveryCount').textContent=clientBrief?(allResults.length-excluded).toLocaleString()+' sites need fit review · '+excluded.toLocaleString()+' excluded · '+catalog.length.toLocaleString()+' in catalog':results.length.toLocaleString()+' sites · '+rows.length.toLocaleString()+' source records · '+catalog.length.toLocaleString()+' in catalog';
