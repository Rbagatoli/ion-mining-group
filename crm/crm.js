(function(){
  ...
  let current='control',selection=null,siteTab='overview',pipelineKind='all',pipelineGroup='active',pipelineSearch='',pipelineLimit=60,pipelineWorkflow='all',teamTab='workflow',taskFilter='open',taskRole='',month=day().slice(0,7),peopleSearch='',modalForm=false,modalBack=null,focusBefore=null,busy=false,noticeTimer,renderQueued=false;
  ...
  function renderPipeline(){
    ...
    return head('Your pipeline.','See where each opportunity stands and what happens next.','add','Add new')+(pipelineKind==='lead'?F.overview(state,{selected:pipelineWorkflow}):'')+
      ...
      (pipelineWorkflow!=='all'?'<p class="wf-filter-note">Showing '+esc((F.steps.find(s=>s[0]===pipelineWorkflow)||[])[1]||pipelineWorkflow)+'<button class="text-button" data-action="workflow-stage" data-id="all">Clear stage filter</button></p>':'')+'<div class="segmented" aria-label="Deal progress">'+M.groups.map(g=>'<button data-action="pipeline-group" data-id="'+g.id+'" aria-pressed="'+(pipelineGroup===g.id)+'">'+g.label+'</button>').join('')+'</div><p class="result-count">'+filtered.length+' records'+(pipelineGroup==='active'?' · Closed history is available under All records or Closed.':pipelineGroup==='all'?' · Includes closed history.':'')+'</p><section class="panel" id="pipelineList">'+
  ...
  function leadForm(id){
    ...
    form(id?'Update revenue lead':'New revenue lead',prepared+field('company','Company',l.company,'text','required maxlength="180"')+field('website','Company website',l.website,'url','required')+pair(select('offer','Service',Object.assign({'':'Choose a service'},A.OFFERS),l.offer||''),select('stage','Lead stage',A.LEAD_STAGES,l.stage||'discovered'))+
      ...
      area('serviceFit','Why would they buy this service?',l.serviceFit||'','maxlength="2000"')+'<p class="quiet-note">Describe a concrete need this service can address. ASIC buyers need dated evidence of current unmet equipment demand, a relevant reachable contact and a reason to use Proton. Major institutional miners and captive procurement are excluded, including Hut 8 and American Bitcoin / ABTC.</p><p class="quiet-note">Keep unsupported candidates in assignment research notes. For an existing ineligible or stale record, select Not a fit and retain the reason, evidence and material-change trigger in Notes. A reminder alone does not renew demand.</p>'+field('nextAction','Next action',l.nextAction)+field('due','Next action due',l.due,'date')+
  ...
  function sourcingForm(leadId,parentId){
    ...
    if(!eligible.length){modal('Start with a buyer lead','<p>Add an eligible buyer with the Sourcing Desk service only when there is dated evidence of current unmet equipment need, a relevant reachable contact and a reason Proton can help. Keep unsupported candidates in assignment research notes and missing details unknown.</p>'+button('Add revenue lead','new-lead'));return;}
  ...
  function leadInfo(l){
    ...
    const followup=outreach.unknown?'Reconcile the unknown contact outcome before another attempt.':suppressed||outreach.suppressed?'Proactive contact is suppressed. Preserve the recorded opt-out.':l.stage==='disqualified'?'No active follow-up. Preserve the closure reason and evidence in the lead notes.':outreach.paused?'Review the actual reply and continue only within its requested scope.':outreach.parked?'Prospecting is parked. Wait for evidenced re-engagement.':l.nextAction||'Check current unmet need, service fit and the right contact.';
    ...
    const evidence='<p class="group-label">Reason to buy this service</p><p class="note-text">'+esc(l.serviceFit||'Not established.')+'</p><p class="group-label">Buying signal</p><p class="note-text">'+esc(l.signal||'Not researched yet.')+'</p><p class="quiet-note">'+external(l.source,'View signal source')+'</p>';
  ...
  async function act(action,id){
    ...
    if(action==='cc-pipeline'||action==='cc-stage'){pipelineWorkflow='all';pipelineKind=action==='cc-pipeline'&&['site','lead','deal'].includes(id)?id:'all';pipelineGroup=action==='cc-stage'?id:'active';pipelineSearch='';pipelineLimit=60;location.hash='#pipeline';if(current==='pipeline')render();return;}
    ...
    if(action==='add')return addPicker();if(action==='go-discover'){close();location.hash='#discover';return;}if(action==='go-pipeline'){pipelineWorkflow='all';pipelineKind='all';pipelineGroup='active';pipelineSearch='';pipelineLimit=60;location.hash='#pipeline';if(current==='pipeline')render();return;}
