...
(function(){
    ...
    function change(event){selection=event.detail||selection;preview();if(scene){try{scene.setModel(selection.modelKey,modelVariant());sync();}catch(_){fallback();}}}
    async function load(){
    ...
    controls.addEventListener('click',event=>{const action=event.target.closest('[data-br-view]')?.dataset.brView;if(!action||!scene)return;
        if(action==='play'){playing=!playing;sync();return;}pause();if(action==='in')scene.zoom(.84);if(action==='out')scene.zoom(1.2);if(action==='reset')scene.reset();});
    controls.addEventListener('focusin',event=>{if(event.target!==play)pause();});
    media.addEventListener('change',()=>{playing=false;sync();});window.addEventListener('brokerage:model',change);
