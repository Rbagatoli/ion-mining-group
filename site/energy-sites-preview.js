...
(function () {
  ...
  // This preview owns its live copy. Re-render only its current view when the
  // phone breakpoint changes; source, site and tab selections remain in state.
  const phoneMedia = window.matchMedia('(max-width: 640px)');
  let printing = false;
  const compact = (desktop, phone) => phoneMedia.matches && !printing ? phone : desktop;
  const sourceNote = 'Landfill and stranded gas are our specialty. Other routes receive focused research when your brief or a site-specific opportunity calls for it.';
  const shortIntros = new Map([
    ['Match a flexible mining load to the output you can actually offer.', 'Match flexible mining demand to supply actually available.'],
    ['Start with the commercial supply route and an appropriate location for the load.', 'Confirm the supply route and suitable location.'],
    ['Confirm electrical output and the supply arrangement around the existing plant.', 'Confirm plant output and supply terms.'],
    ['Design around the output profile and the hours when flexible demand adds value.', 'Match operating hours to available output.'],
    ['Compare a potential power allocation with the plant’s obligations and operating costs.', 'Check allocations, existing commitments and operating costs.'],
    ['Scope the fuel-to-power route before sizing the mining load.', 'Scope fuel conversion before sizing miners.'],
    ['Establish usable electrical output and what conversion equipment is actually in place.', 'Verify net output and installed conversion equipment.'],
    ['Use the headroom that the owner can make available without disrupting the main operation.', 'Verify spare capacity without disrupting site operations.'],
    ['Assess the full cost and conditions of serving a new mining load.', 'Check total service cost and conditions.']
  ]);
  ...
  function panel(p, a) {
    if (activeTab === 'capital') return `<div class="ep-panel-intro"><span class="ep-kicker">Spend where it is needed</span><h4>What is built. What remains.</h4></div><div class="ep-evidence"><span class="ep-badge">Reported infrastructure</span><p>${esc(p.infrastructureSummary)}</p></div><div class="ep-capital"><div><small>Remaining build cost</small><strong>Not yet priced</strong></div><div><small>Verified reuse savings</small><strong>Not established</strong></div></div><p class="ep-copy">${esc(p.capitalSummary)}</p><div class="ep-next"><strong>Before counting a saving</strong><p>${esc(compact("Confirm equipment ownership, condition, spare capacity and permission to use it. Then price the connection, upgrades and missing works.", "Verify ownership, condition, spare capacity and reuse rights. Price connections, upgrades and missing work."))}</p></div>${sourceLink('../portal/scouting/#sites', 'Open the full sample report')}`;
    if (activeTab === 'contacts') return `<div class="ep-panel-intro"><span class="ep-kicker">The next conversation</span><h4>People. Roles. Contact routes.</h4><p>${esc(compact("Published business details; owner interest has not been confirmed.", "Published contacts; owner interest unconfirmed."))}</p></div><div class="ep-contacts">${(a?.contacts || []).slice(0,2).map(c => `<article><strong>${esc(c.name)}</strong><span>${esc(c.role)} · ${esc(c.organization)}</span><div>${c.phone ? `<a href="${esc(phoneHref(c.phone))}">${esc(c.phone)}</a>` : ''}${c.email ? `<a href="mailto:${esc(c.email)}">${esc(c.email)}</a>` : ''}</div><small>${esc(c.verification)}</small>${sourceLink(c.sourceUrl, 'Published source')}</article>`).join('')}</div><p class="ep-copy">${esc(compact("The full workspace includes further contact routes, questions to ask and evidence behind each lead.", "More contacts, questions and evidence in the full workspace."))}</p>`;
  ...
  function sourcePanel(s) {
    const guide = phoneMedia.matches && !printing ? (s.mobile || s) : s;
    if (activeTab === 'capital') return '<div class="ep-panel-intro"><span class="ep-kicker">Protect the upfront budget</span><h4>Start with reusable infrastructure.</h4></div><div class="ep-evidence"><span class="ep-badge">What to investigate</span><p>' + esc(guide.assets) + '</p></div><div class="ep-capital"><div><small>Existing assets</small><strong>Verify before crediting</strong></div><div><small>Remaining capital</small><strong>Scope site by site</strong></div></div><div class="ep-next"><strong>What goes into the remaining budget</strong><p>' + esc(compact("Connection and metering, equipment upgrades, cooling and mining fitout, site works, and any missing generation or storage. Separate your share from owner-funded work.", "Price connections, metering, upgrades, cooling, mining fitout, site works and missing generation or storage. Separate owner funding.")) + '</p></div><p class="ep-copy">' + esc(compact("The site comparison identifies evidence, estimates and unpriced items. Savings depend on usable capacity, condition, ownership and access.", "Separate evidence, estimates and open costs. Savings require usable capacity, condition, ownership and access.")) + '</p>';
    if (activeTab === 'contacts') {
      const c = contactPaths[s.id];
      return '<div class="ep-panel-intro"><span class="ep-kicker">Qualify the opportunity</span><h4>Reach the people who control access.</h4><p>' + esc(compact("For each researched site, we look for relevant individuals, published business phones and emails, and the sources behind them.", "Find relevant people, published business phones, emails and sources.")) + '</p></div><div class="ep-contact-path">' + [0,2].map((i,n) => '<article><span>' + (n+1) + '</span><div><strong>' + esc(c[i]) + '</strong><p>' + esc(c[i+1]) + '</p></div></article>').join('') + '</div><div class="ep-next"><strong>The first conversation</strong><p>' + esc(compact("Is the owner open to a mining load? What net MW, delivery point, operating hours and price could they offer—and who can approve the arrangement?", "Confirm mining interest, net MW, delivery point, hours, price and approval authority.")) + '</p></div><p class="ep-copy">' + esc(compact("These are roles to research. Named contacts and owner responses belong to a specific site report.", "Research roles only. Named contacts and responses are site-specific.")) + '</p>';
    }
    return '<div class="ep-panel-intro"><span class="ep-kicker">Could this source fit your mine?</span><h4 class="ep-source-repeat-title">' + esc(s.label) + '</h4><p>' + esc(compact(s.intro.replace('you can actually offer', 'a supplier can actually offer'), shortIntros.get(s.intro) || s.intro.replace('you can actually offer', 'a supplier can actually offer'))) + '</p></div><ul class="ep-source-checks">' + guide.checks.map((t,i) => '<li><span>0' + (i+1) + '</span>' + esc(t) + '</li>').join('') + '</ul><div class="ep-next"><strong>' + esc(compact("Compare on usable power and total cost", "Compare the whole cost")) + '</strong><p>' + esc(compact("Screen the MW available to your load, delivered energy cost, operating flexibility and capital still needed to connect.", "Check usable MW, delivered cost, operating flexibility and remaining capital.")) + '</p></div><p class="ep-copy">' + esc(compact("Storage and hybrid arrangements are considered with their underlying supply, charging costs and delivery schedule.", "Storage and hybrids require supply, charging costs and a delivery schedule.")) + '</p>';
  }
  function renderSource() {
    ...
    root.querySelector('.ep-detail-heading').innerHTML = '<span class="ep-kicker">' + esc(s.route) + ' · ' + (specialty ? 'Our sourcing specialty' : 'Selective research') + '</span><h3>' + esc(s.label) + '</h3><span class="ep-badge">' + esc(compact('Source guide · site availability to be researched', 'Source guide · Availability unconfirmed')) + '</span>';
  ...
  function render() {
    root.dataset.previewView = mode;
    root.querySelector('[data-preview-mode="sources"]').textContent = compact('Explore energy sources', 'Energy guide');
    root.querySelector('[data-preview-mode="research"]').textContent = compact('Researched landfill examples', 'Site examples');
    root.querySelector('.ep-source-note').textContent = compact(sourceNote, 'Gas is our specialty. Other sources researched to your brief.');
  ...
  function renderPanel(p, a, source) {
    root.dataset.previewPanel = activeTab;
  ...
  const refreshLayout = () => {
    ...
    const activeTabId = focused?.matches('[data-preview-tab]') ? focused.dataset.previewTab : null;
    render();
    if (activeTabId) root.querySelector('#ep-tab-' + activeTabId)?.focus({ preventScroll: true });
  };
  if (phoneMedia.addEventListener) phoneMedia.addEventListener('change', refreshLayout);
  else phoneMedia.addListener(refreshLayout);
  window.addEventListener('beforeprint', () => { printing = true; refreshLayout(); });
  window.addEventListener('afterprint', () => { printing = false; refreshLayout(); });
  render();
}());
