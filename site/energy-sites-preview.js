...
(function () {
  ...
  let mode = sources.length ? 'sources' : 'research', sourceId = 'hydro';
  ...
  const quickSources = ['hydro','nuclear','wind','solar','natural_gas','industrial_surplus','grid_supply','landfill_gas'];
  const sourceRoot = root.querySelector('.ep-source-explorer');
  sourceRoot.innerHTML = '<label class="ep-source-select" for="ep-source">Choose an energy source<select id="ep-source">' + sources.map(s => '<option value="' + esc(s.id) + '">' + esc(s.label) + '</option>').join('') + '</select></label><div class="ep-source-grid" aria-label="Common energy sources">' + quickSources.map(id => { const s = sources.find(s => s.id === id); return s ? '<button type="button" data-preview-source="' + id + '" aria-pressed="false">' + esc(s.label) + '</button>' : ''; }).join('') + '</div><div class="ep-source-route"></div><p class="ep-source-note">Nationwide research to your brief. Specific locations and contacts are added as sites are researched.</p>';
  ...
  function renderSource() {
    ...
    root.querySelector('.ep-brief').innerHTML = '<span>YOUR SEARCH</span><strong>Nationwide · Matched to your requirements</strong><span>Existing infrastructure prioritized</span>';
    root.querySelector('.ep-detail-heading').innerHTML = '<span class="ep-kicker">' + esc(s.route) + ' · Nationwide sourcing</span><h3>' + esc(s.label) + '</h3><span class="ep-badge">Source guide · site availability to be researched</span>';
