...
(function(root,factory){
  ...
  function buildBrief(values){
    ...
    const lines=[buy?'PROTON MINING — BUYER REQUIREMENTS':'PROTON MINING — ASIC SELLER BRIEF','PREPARED ONLY — NOT SUBMITTED','',
      ...
      (buy?'Purchase timing: ':'Availability / timing: ')+(clean(values.timing)||'Not provided'),
    ...
    if(!buy)lines.push('Proposed seller fee: 3% of the completed sale price; final scope and fees agreed before representation. Shipping, inspection and other agreed costs are separate.');
    return lines.join('\n');
  }
  function mount(doc){
