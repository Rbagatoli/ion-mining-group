...
const EXPERIENCE_BEGIN = '<!-- hosting-experience:begin -->';
const EXPERIENCE_END = '<!-- hosting-experience:end -->';
...
function hostingExperience() {
    ...
    const details = '        <div class="hx-place"><span data-globe="region">' + esc(first.region) + '</span><h4 data-globe="name">' + esc(first.name) + '</h4><p data-globe="fuel">' + esc(first.fuel) + '</p></div>\n' +
        '        <dl class="hx-metrics"><div><dt>Site capacity</dt><dd data-globe="capacity">' + esc(Facilities.capacityLabel(first)) + '</dd></div><div><dt data-mobile-copy="Est. rate">Hosting rate</dt><dd data-globe="rate">' + esc(Facilities.powerLabel(first)) + '</dd></div><div><dt>Availability</dt><dd class="hx-status" data-globe="status">' + esc(first.status) + '</dd></div></dl>\n' +
...
function build() {
    ...
    const ta = next.indexOf(EXPERIENCE_BEGIN), tb = next.indexOf(EXPERIENCE_END);
    if (ta < 0 || tb < ta) throw new Error('hosting.html: hosting experience markers missing or out of order');
    next = next.slice(0,ta) + EXPERIENCE_BEGIN + '\n' + hostingExperience() + '\n    ' + EXPERIENCE_END + next.slice(tb+EXPERIENCE_END.length);
