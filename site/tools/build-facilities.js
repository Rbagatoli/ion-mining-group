...
function hostingExperience() {
    ...
    const details = '        <div class="hx-place"><span data-globe="region">' + esc(first.region) + '</span><h4 data-globe="name">' + esc(first.name) + '</h4><p data-globe="fuel">' + esc(first.fuel) + '</p></div>\n' +
        '        <dl class="hx-metrics"><div><dt>Site capacity</dt><dd data-globe="capacity">' + esc(Facilities.capacityLabel(first)) + '</dd></div><div><dt>Hosting rate</dt><dd data-globe="rate">' + esc(Facilities.powerLabel(first)) + '</dd></div><div><dt>Availability</dt><dd class="hx-status" data-globe="status">' + esc(first.status) + '</dd></div></dl>\n' +
